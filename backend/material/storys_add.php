...
if(isset($_POST['submit'])){
    ...
    $story_type_id = $_POST['story_type_id'];
    ...
    if(!empty($id)){
        $data = "category_id='{$category_id}',story_type_id='{$story_type_id}',name='{$name}',description='{$description}',image='{$file_name}'";
        $where = "id=".$id;
        include 'update_data.php';
    }else{
        $column = 'category_id,story_type_id,name,description,image';
        $data = "'{$category_id}','{$story_type_id}','{$name}','{$description}','{$file_name}'";
            ...
            <div class="card-body">
                ...
                <form  method="post" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control custom-select" name="story_type_id" required data-validation-required-message="This field is required">
                            <option value="">Select</option>
                            <?php 
                            $sql3 = "SELECT * FROM tblstory_type ";
